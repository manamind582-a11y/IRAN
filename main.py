...
TOPUP_MIN      = 10000     # حداقل مبلغ افزایش اعتبار (تومان)
...
# ─ تلگرام ─
_TG_USER = r"[A-Za-z0-9_]{5,32}"

def validate_tg_link(link):
    """https://t.me/username  (کانال یا گروه عمومی، بدون بخش اضافه)"""
    return bool(re.fullmatch(r"(?:https?://)?(?:www\.)?t\.me/" + _TG_USER + r"/?(?:\?\S*)?", link.strip(), re.I))

def validate_tg_post_link(link):
    """https://t.me/username/1234"""
    return bool(re.fullmatch(r"(?:https?://)?(?:www\.)?t\.me/" + _TG_USER + r"/\d+/?(?:\?\S*)?", link.strip(), re.I))

def validate_tg_private_link(link):
    """https://t.me/+xxxxxxxx  یا  https://t.me/joinchat/xxxxxxxx"""
    link = link.strip()
    if re.fullmatch(r"(?:https?://)?(?:www\.)?t\.me/\+[A-Za-z0-9_-]{10,}/?", link, re.I):
        return True
    return bool(re.fullmatch(r"(?:https?://)?(?:www\.)?t\.me/joinchat/[A-Za-z0-9_-]{10,}/?", link, re.I))

def validate_tg_link_or_private(link):
    return validate_tg_link(link) or validate_tg_private_link(link)

def validate_tg_username(link):
    """@username یا username یا لینک t.me/username — آیدی گیرنده (برای استارز، پرمیوم، گیفت)"""
    link = link.strip()
    if re.fullmatch("@?" + _TG_USER, link):
        return True
    return validate_tg_link(link)

def validate_tg_auto_view_setup(link):
    """سه خط: یوزرنیم کانال (بدون @) / تعداد پست‌های جدید / تعداد پست‌های قدیمی"""
    ls = [l.strip() for l in link.strip().split("\n") if l.strip()]
    if len(ls) != 3:
        return False
    user, new_p, old_p = ls
    return bool(re.fullmatch(_TG_USER, user.lstrip("@"))) and new_p.isdigit() and old_p.isdigit()
...
def send_account(chat_id):
    ...
    send(chat_id,
        ...
        f"✅ تکمیل شده: {done}*",
        kb(["💳 افزایش اعتبار"], ["🔙 بازگشت به منوی اصلی 🏠"])
    )
...
def start_topup(chat_id):
    """شروع فرآیند افزایش اعتبار؛ اگر نام و شماره قبلاً ثبت شده مستقیم سراغ مبلغ می‌رود."""
    data = load_data()
    u = data["users"].get(str(chat_id), {})
    if u.get("full_name") and u.get("phone"):
        states[str(chat_id)] = "topup_amount"
        send(chat_id,
            f"*💳 افزایش اعتبار کیف پول\n\n"
            f"👤 {u['full_name']} | 📞 {u['phone']}\n\n"
            f"💰 مبلغ مورد نظر را به تومان بنویسید (حداقل {TOPUP_MIN:,} تومان):*",
            BACK_BTN_USER)
    else:
        states[str(chat_id)] = "topup_fullname"
        send(chat_id,
            "*💳 افزایش اعتبار کیف پول\n\n"
            "قبل از شارژ، یک بار اطلاعات تماس‌تون رو می‌گیریم تا اگه مشکلی پیش اومد بتونیم باهاتون تماس بگیریم و "
            "موجودی‌تون سوخت نشه.\n\n"
            "👤 لطفاً نام و نام خانوادگی خودتون رو بنویسید:*",
            BACK_BTN_USER)

def send_topup_invoice(chat_id, amount):
    data = load_data()
    data["topup_counter"] = data.get("topup_counter", 0) + 1
    tid = data["topup_counter"]
    data.setdefault("topups", []).append({
        "id": tid, "user_id": chat_id, "amount": amount,
        "status": "pending_payment", "date": datetime.now().isoformat()
    })
    save_data(data)
    result = requests.post(f"{BASE_URL}/sendInvoice", json={
        "chat_id":        chat_id,
        ...
        "payload":        f"topup_{tid}",
        "provider_token": PROVIDER_TOKEN,
        "currency":       "IRR",
        "prices":         [{"label": "افزایش اعتبار", "amount": amount * 10}],
        "start_parameter": "pay"
    }, timeout=15).json()
    print(f"TOPUP INVOICE RESULT = {result}")
    if not result.get("ok"):
        send(chat_id, "*❌ خطا در ارسال فاکتور! با پشتیبانی تماس بگیرید: @ARKA_SUPPORT_IR*")

def register_successful_topup(chat_id, payload):
    data = load_data()
    try:
        tid = int(payload.replace("topup_", ""))
    except Exception:
        tid = None
    target = None
    for t in data.get("topups", []):
        if t["id"] == tid and t["user_id"] == chat_id and t["status"] == "pending_payment":
            target = t; break
    if not target:
        for t in reversed(data.get("topups", [])):
            if t["user_id"] == chat_id and t["status"] == "pending_payment":
                target = t; break
    if not target:
    ...
    u = data["users"].setdefault(uid, {
        "chat_id": chat_id, "first_seen": datetime.now().isoformat(), "last_seen": datetime.now().isoformat(),
        "orders": 0, "total_spent": 0, "blocked": False, "username": "", "first_name": "",
        "wallet": 0, "full_name": "", "phone": ""})
    u["wallet"] = u.get("wallet", 0) + target["amount"]
    save_data(data)
    send(chat_id,
        f"*✅ کیف پولتون شارژ شد!\n\n"
        f"💰 مبلغ: {target['amount']:,} تومان\n"
        f"💼 موجودی جدید: {u['wallet']:,} تومان\n\n"
        f"می‌تونید موقع خرید از کیف پولتون استفاده کنید. 🌷*",
        BACK_BTN_USER)
    for aid in ADMIN_IDS:
        try:
            send(aid,
                f"*💳 افزایش اعتبار موفق\n\n"
                f"👤 {u.get('full_name') or u.get('first_name','?')} | 📞 {u.get('phone','?')}\n"
                f"🆔 {chat_id}\n"
                f"💰 {target['amount']:,} تومان\n"
                f"💼 موجودی جدید: {u['wallet']:,} تومان*")
        except Exception:
            pass
...
RESERVED_TITLES = {
    ...
    "❌ بستن تیکت", "❌ لغو", "مشاهده و خرید خدمات 🛍", "حساب کاربری 👤", "💳 افزایش اعتبار",
...
# ══════════════════════════════════════════
#  تلگرام  (قیمت‌ها دو برابر ثبت شده - همه‌ی محصولات)
# ══════════════════════════════════════════
def _tg(key, title, label, price, mn, mx, body, platform, x=2, fixed=False, icon=None, auto_view=False):
    final = price * x
    desc = body if fixed else (_hdr(label, final) + "\n\n" + body)
    return dict(key=key, title=title, label=label, price=final, mn=mn, mx=mx, desc=desc, platform=platform,
                fixed=fixed, icon=icon, auto_view=auto_view)


_TG_NO_ORDER_TOGETHER = "📌 سفارش همزمان برای یک لینک از یک سرویس ثبت نکنید."
_TG_IRAN_ACC = "گروه را حتما باید با شماره (اکانت) ایرانی +98 ساخته شده باشد"
_TG_IRAN_ACC_CH = "کانال را حتما باید با شماره (اکانت) ایرانی +98 ایجاد کنید"
_TG_BANNED_TOPICS = "📌گروه های مخدر،پورن،صیغه،دوستیابی،طلسم،فال،دعا، وام و.. انجام نمی‌شود"
_TG_BURN_WARN = "⚠️ در صورت رعایت نکردن توضیحات سرویس، مبلغ شما سوخت می‌شود"

def _L(*lines):
    return "\n".join(lines)

# ─ ممبر ایرانی | گروه تلگرام ─
def _tg_group_ir_desc(start):
    return _L(_TG_IRAN_ACC, "", f"🕑 زمان شروع : {start}", "🩸 میزان ریزش : واقعی ، نامشخص", "👌 کیفیت سرویس : ایرانی",
              "✅ زمان تکمیل : 24 تا 72 ساعت", "🔗 نمونه لینک : https://t.me/username", "",
              "نکات مهم :", "", "📌افزودن عضو در گروه باز باشد", "📌گروه عمومی باشد.", _TG_BANNED_TOPICS,
              _TG_BURN_WARN)

TG_GROUP_IR = [
    _tg("tg_grp_hidden", "👤 ممبر هیدن گروه", "ممبر هیدن گروه تلگرام", 135200, 1000, 50000,
        _tg_group_ir_desc("6 الی 12 ساعت"), "tg_link"),
    _tg("tg_grp_adlist_low", "🐢 ادلیست گروه | سرعت پایین", "ادلیست گروه تلگرام -- سرعت پایین", 190800, 1000, 100000,
        _tg_group_ir_desc("6 الی 12 ساعت"), "tg_link"),
    _tg("tg_grp_adlist_rec", "⭐ ادلیست گروه | پیشنهادی", "ادلیست گروه تلگرام - پیشنهادی", 353100, 1000, 200000,
        _tg_group_ir_desc("6 الی 12 ساعت"), "tg_link"),
    _tg("tg_grp_ir_notes", "📋 ممبر ایرانی گروه {توضیحات بخونید}", "ممبر ایرانی گروه تلگرام {توضیحات بخونید}", 640000, 1000, 80000,
        _L("🕑 زمان تکمیل : 48 الی 72 ساعت", "🩸 میزان ریزش : به دلیل واقعی بودن نامشخص است",
           "👌 کیفیت سرویس : ایرانی", "🔗 نمونه لینک : لینک گروه عمومی را وارد کنید", "",
           "نکات مهم :",
           "1- بایستی گزینه ادد ممبر گروه فعال باشد",
           "2- تایید مدیر برای عضو شدن در گروه برای اعضا جدید ، غیر فعال باشد",
           "3- گروه باید پابلیک(عمومی)باشد. اگر گروه شما پرایوت(شخصی) باشد و سفارش دهید ممبر واریز نمیشود و وجه نیز برگشت نمی خورد.",
           "4- لینک عمومی گروه را وارد کنید مثال: https://t.me/marketing98",
           "5- برای گروه های خارجی سفارش ندهید.(سازنده ی گروه بایستی با شماره ی ایرانی گروه را ساخته باشد)"),
        "tg_link"),
]

# ─ ممبر کانال / گروه خارجی تلگرام ─
def _tg_foreign_desc(start, speed, drop, quality, gift=None, extra_notes=None):
    lines = [f"🕑 زمان شروع : {start}", f"🚀 سرعت ارسال : {speed}", f"🩸 میزان ریزش : {drop}"]
    if gift:
        lines.append(f"🎁 هدیه : {gift}")
    lines += [f"👌 کیفیت سرویس : {quality}", "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : https://t.me/username",
              "", "نکات مهم :", ""]
    lines += extra_notes or ["📌کانال عمومی باشد.", "📌کانال در حال ریزش نباشد. مسئولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد."]
    return _L(*lines)

TG_FOREIGN = [
    _tg("tg_foreign_30d", "⚡ ممبر تلگرام | ۳۰ روز بدون ریزش", "ممبر تلگرام[30 روز بدون ریزش]⚡", 131600, 100, 300000,
        _tg_foreign_desc("1 الی 3 ساعت", "10 الی 20 کا در 24 ساعت", "30 روز بدون ریزش", "مناسب و خارجی"), "tg_link"),
    _tg("tg_foreign_90d", "🔥 ممبر تلگرام | ۹۰ روز بدون ریزش", "ممبر تلگرام {90 روز بدون ریزش }🔥", 241800, 100, 100000,
        _tg_foreign_desc("1 الی 5 ساعت", "5 تا 20 کا در 24 ساعت", "90 روز بدون ریزش", "خارجی", gift="0 الی 10%",
                          extra_notes=["📌لینک را به درستی ثبت کنید و کانال عمومی باشد.",
                                      "📌این سرویس برای سفارش همزمان کار نمی کند؛ سفارش همزمان برای یک لینک از این سرویس ثبت نکنید تا سفارش شما تکمیل شده و مجدد ثبت سفارش انجام دهید.",
                                      "📌در صورتی که در حین ارسال سفارش و یا بعد از ثبت سفارش لینک تغییر داده شود وضعیت سفارش به تکمیل شده تغییر کرده و هزینه برگشت داده نمی شود.",
                                      "📌در صورت ریزش ممبر های قبلی جبران ریزش ندارد."]), "tg_link"),
    _tg("tg_foreign_180d", "⭐ ممبر تلگرام | ۱۸۰ روز بدون ریزش", "ممبر تلگرام {180 روز بدون ریزش }⚡️⭐", 369800, 500, 30000,
        _tg_foreign_desc("1 الی 5 ساعت", "5 تا 20 کا در 24 ساعت", "180 روز بدون ریزش", "خارجی", gift="0 الی 10%",
                          extra_notes=["📌لینک را به درستی ثبت کنید و کانال عمومی باشد.",
                                      "📌این سرویس برای سفارش همزمان کار نمی کند؛ سفارش همزمان برای یک لینک از این سرویس ثبت نکنید تا سفارش شما تکمیل شده و مجدد ثبت سفارش انجام دهید.",
                                      "📌در صورتی که در حین ارسال سفارش و یا بعد از ثبت سفارش لینک تغییر داده شود وضعیت سفارش به تکمیل شده تغییر کرده و هزینه برگشت داده نمی شود.",
                                      "📌در صورت ریزش ممبر های قبلی جبران ریزش ندارد."]), "tg_link"),
...
# ─ ممبر و بازدید ایرانی تلگرام ─
def _tg_hidden_ch_desc(mx_text):
    return _L(_TG_IRAN_ACC_CH, "", "🕑 زمان شروع : 6 الی 12 ساعت", "🩸 میزان ریزش : واقعی ، نامشخص",
              "👌 کیفیت سرویس : ایرانی", "✅ زمان تکمیل : 24 تا 72 ساعت", "🔗 نمونه لینک : https://t.me/username", "",
              "نکات مهم :", "", _TG_NO_ORDER_TOGETHER, "📌کانال عمومی باشد.", "📌موضوع کانال خلاف قوانین کشور نباشد")

TG_IR_MEMBER_VIEW = [
    _tg("tg_hidden_ch_100k", "👤 ممبر هیدن کانال | زیر ۱۰۰ کا", "ممبر هیدن کانال تلگرام زیر 100 کا", 111400, 1000, 50000,
        _tg_hidden_ch_desc("100"), "tg_link"),
    _tg("tg_hidden_ch_200k", "👤 ممبر هیدن کانال | زیر ۲۰۰ کا", "ممبر هیدن کانال تلگرام زیر 200 کا", 127300, 1000, 50000,
        _tg_hidden_ch_desc("200"), "tg_link"),
    _tg("tg_forced_ch", "🔒 ممبر اجباری کانال", "ممبر اجباری کانال تلگرام", 239800, 1000, 20000,
        _tg_hidden_ch_desc("20"), "tg_link"),
    _tg("tg_member_ir_ch", "🇮🇷 ممبر ایرانی کانال", "ممبر ایرانی کانال تلگرام", 293700, 1000, 100000,
        _L("🕑 زمان شروع : 1 الی 2 ساعت", "🚀 سرعت ارسال : 200 الی 1 کا در 24 ساعت",
           "🩸 میزان ریزش : به دلیل واقعی بودن بستگی به مخاطب دارد", "👌 کیفیت سرویس : ایرانی",
           "🔗 نمونه لینک : لینک کانال را وارد کنید", "",
           "نکات مهم :",
           "📌 ثبت سفارش فقط بصورت اعداد رُند مانند ۳۰۰۰ - ۴۰۰۰ - ۵۰۰۰ امکان پذیر می باشد؛ در صورت وارد کردن اعداد بصورت غیر از آنچه گفته شده (مثلا : 3200) سفارش دچار مشکل خواهد شد و هزینه برگشت داده نخواهد شد.",
           "📌کانال عمومی باشد.", "📌جبران ریزش ندارد", "📌سفارش برای کانال های با مالکیت ایرانی (۹۸+) امکان پذیر می باشد",
           "📌سفارش برای کانال هایی مانند صیغه یابی و هکینگ و یا مواردی که خلاف قوانین می باشد قابل ثبت نمی باشد."),
        "tg_link"),
    _tg("tg_uploader_rec", "🔥 ممبر آپلودر | پیشنهادی", "ممبر آپلودر- پیشنهادی🔥", 470300, 1000, 200000,
        _L("🕑 زمان شروع : 3 الی 12 ساعت", "🩸 میزان ریزش : به علت واقعی بودن نامشخص", "✅ زمان تکمیل : 24 تا 72 ساعت",
           "🔗 لینک : لینک خصوصی کانال یا گروه", "", "https://t.me/+7iQtWEwbStg4YTdk", "",
           "نکات مهم :",
           "📌 ربات @AdUplaoderiBot باید فول‌ادمین باشد.",
           "📌 شماره سازنده کانال یا گروه ایرانی و تا اتمام سفارش کانال گروه عمومی باشد",
           "📌 " + _TG_BANNED_TOPICS[1:], "📌 هنگام ثبت و انجام سفارش لینک تغییر نکند",
           "📌 تغییرلینک حین انجام کار مبلغ سوخت می‌شود و سفارش بدون انجام تکمیل میگرد",
           "📌 همزمان در دو سایت و ربات سفارش ثبت نکنید دچار اختلال میشود.", "",
           "⚠️ در صورت رعایت نکردن توضیحات و قوانین محصول ، هیچ گونه تعهد و مسئولیتی بابت سفارش شما نداریم و مبلغ شما سوخت می‌شود !"),
        "tg_private_link"),
    _tg("tg_member_ir_grp", "🇮🇷 ممبر ایرانی گروه", "ممبر ایرانی گروه تلگرام", 303800, 1000, 100000,
        _L("🕑 زمان شروع : 1 الی 2 ساعت", "🚀 سرعت ارسال : 200 الی 1 کا در 24 ساعت",
           "🩸 میزان ریزش : به دلیل واقعی بودن بستگی به مخاطب دارد", "👌 کیفیت سرویس : ایرانی",
           "🔗 نمونه لینک : لینک گروه را وارد کنید", "📌 گروه عمومی ثبت کنید ادد گروه باز باشد .",
           "⚠️ برای گروه خارجی و مالک مجازی ثبت نکنید پولتون از دست میدید"), "tg_link"),
    _tg("tg_member_ir_100k", "🇮🇷 ممبر ایرانی | کانال زیر ۱۰۰ کا", "ممبر ایرانی تلگرام (کانال های زیر 100 کا)", 500000, 3000, 80000,
        _L("🕑 زمان تکمیل : 48 الی 72 ساعت", "🩸 میزان ریزش : به دلیل واقعی بودن نامشخص است", "👌 کیفیت سرویس : ایرانی",
           "🔗 نمونه لینک : لینک کانال را وارد کنید", "",
           "نکات مهم :", "📌کانال عمومی باشد.", "📌سفارش برای کانال های با مالکیت ایرانی (۹۸+) امکان پذیر می باشد"),
        "tg_link"),
    _tg("tg_uploader_200k", "📦 ممبر آپلودر (کانال و گروه) زیر ۲۰۰ کا", "ممبر آپلودر ( کانال و گروه ) زیر 200کا", 564300, 1000, 200000,
        _L(_TG_IRAN_ACC_CH, "", "🕑 زمان شروع : 3 الی 12 ساعت", "🚀 سرعت ارسال : 10 الی 20 کا در 24 ساعت",
           "🩸 میزان ریزش : به علت واقعی بودن ممبر نامشخص", "💎 کیفیت : آنلاین، ویودار، واقعی، ایرانی",
           "✅ زمان تکمیل : 24 تا 48 ساعت",
           "🔗 نحوه ثبت لینک : لینک خصوصی کانال یا گروه خود را مانند نمونه پایین ارسال کنید", "",
           "https://t.me/+7iQtWEwbStg4YTdk", "",
           "نکات مهم :",
           "📚 قوانین محصول:",
           "اگر این ربات @SocialAdderBot فول‌ادمین کانال یا گروه شما نباشد سفارش انجام نمی‌شود",
           "شماره سازنده کانال یا گروه ایرانی و تا اتمام سفارش کانال گروه عمومی باشد",
           _TG_BANNED_TOPICS[1:], "بیشتر از ظرفیت محصول سفارش ثبت نکنید",
           "هنگام ثبت و انجام سفارش لینک تغییر نکند",
           "تغییرلینک حین انجام کار مبلغ سوخت می‌شود و سفارش بدون انجام تکمیل میگرد",
           "همزمان در دو سایت و ربات سفارش ثبت نکنید دچار اختلال میشود.", "",
           "⚠️ در صورت رعایت نکردن توضیحات و قوانین محصول ، هیچ گونه تعهد و مسئولیتی بابت سفارش شما نداریم و مبلغ شما سوخت می‌شود !"),
        "tg_private_link"),
    _tg("tg_member_ir_101k", "🇮🇷 ممبر ایرانی | کانال بالای ۱۰۰ کا", "ممبر ایرانی تلگرام (کانال های بالای 100 کا)", 580000, 3000, 80000,
        _L("🕑 زمان تکمیل : 48 الی 72 ساعت", "🩸 میزان ریزش : به دلیل واقعی بودن نامشخص است", "👌 کیفیت سرویس : ایرانی",
           "🔗 نمونه لینک : لینک کانال را وارد کنید", "",
           "نکات مهم :", "📌کانال عمومی باشد.", "📌سفارش برای کانال های با مالکیت ایرانی (۹۸+) امکان پذیر می باشد"),
        "tg_link"),
...
# ─ اشتراک پرمیوم تلگرام ─ (تعداد ثابت ۱۰۰۰ = یک سفارش)
def _tg_premium_desc(months):
    return _L("🕑 زمان شروع : 0 تا 4 ساعت", "🚀 سرعت تحویل : 1 الی 8 ساعت", f"🩸 ماندگاری : {months} (امکان تمدید دارد)",
              "👌 کیفیت سرویس : فعالسازی روی اکانت شما بدون لاگین", "✅ زمان تکمیل : 24 ساعت",
              "🔗 نمونه لینک : ایدی اکانت تلگرام", "",
              "نکات مهم :", "",
              "📌 عدد 1000 به منزله یک سفارش می باشد؛ موقع ثبت سفارش 1000 را ثبت کنید.",
              "📌 ایدی اکانت خود را به طور صحیح وارد کنید. در صورتی که ایدی اشتباه ثبت شود سفارش برای ایدی اشتباه ثبت شده و سایت هیچ مسئولیتی در قبال این موارد ندارد و هزینه برگشت داده نمی شود.",
              "📌 اکانت شخصی تلگرام شما بصورت کاملا قانونی پریمیوم میشود و اشتراک روی آن فعال میگردد.",
              "📌 با پریمیوم سازی اکانت شخصی، امکان استفاده از تمامی امکانات پلن پریمیوم تلگرام را خواهید داشت .",
              "📌 ثبت سفارش به معنای قبول تمامی نکات و قوانین ذکر شده می باشد.", "",
              "هشدار :", "", "⚠️در صورت ثبت آیدی اشتباه، امکان پیگیری وجود ندارد.")

TG_PREMIUM_SUB = [
    _tg("tg_prem_3m", "3️⃣ پرمیوم ۳ ماهه", "تلگرام پرمیوم 3 ماهه", 3555000, 1000, 1000, _tg_premium_desc("3 ماهه"), "tg_username", fixed=True, icon="💎"),
    _tg("tg_prem_6m", "6️⃣ پرمیوم ۶ ماهه", "تلگرام پرمیوم 6 ماهه", 4740000, 1000, 1000, _tg_premium_desc("6 ماهه"), "tg_username", fixed=True, icon="💎"),
    _tg("tg_prem_12m", "🔟 پرمیوم ۱۲ ماهه", "تلگرام پرمیوم 12 ماهه", 7903900, 1000, 1000, _tg_premium_desc("12 ماهه"), "tg_username", fixed=True, icon="💎"),
]

# ─ استارز تلگرام ─
TG_STARS = _tg("tg_stars", "⭐ استارز تلگرام", "استارز تلگرام ⭐", 4740000, 50, 15000,
    _L("🕑 زمان شروع : 1 تا 24 ساعت", "🚀 سرعت ارسال : بالا", "👌 کیفیت سرویس : با کیفیت",
       "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : آیدی اکانت تلگرام Telegram@", "",
       "نکات مهم :", "", "✅ فعالسازی بر روی اکانت ایران", "✅ قابل تمدید، قانونی، شارژ بدون ورود به اکانت",
       "📌 فقط ایدی اکانت ثبت شود.",
       "📌 میتوانید خدمات بازی ها و انواع سکه‌ی بازی های ارزی رو با این ستاره بخرید و با این نوع سکه میتوان لول بازی های تلگرامی رو در صورت امکان ارتقا داد."),
    "tg_username", icon="⭐")

# ─ ممبر پریمیوم تلگرام [سرچ اول تلگرام باش] ─
def _tg_prem_mem_desc(start, drop, quality, extra_notes=None, gift=None):
    lines = [f"🕑 زمان شروع : {start}", "🚀 سرعت ارسال : 100 الی 500 در 24 ساعت", f"🩸 میزان ریزش : {drop}"]
    if gift:
        lines.append(f"🎁 هدیه : {gift}")
    lines += [f"👌 کیفیت سرویس : {quality}", "✅ زمان تکمیل : 48 تا 72 ساعت",
              "🔗 نحوه درج لینک : لینک با فرمت زیر باشد", "", "https://t.me/xxxxxx", "",
              "نکات مهم :", "", "📌 کانال عمومی باشد"]
    if extra_notes:
        lines += extra_notes
    return _L(*lines)

TG_PREMIUM_MEMBER = [
    _tg("tg_pm_7_10d", "🔟 ممبر پرمیوم | ۷-۱۰ روز", "ممبر پرمیوم [ ۷-۱۰ روز ]", 807200, 100, 6000,
        _tg_prem_mem_desc("1 تا 24 ساعت", "نامشخص", "خارجی (ممبر ها به مدت هفت روز در کانال می مانند)"), "tg_link"),
    _tg("tg_pm_30d", "🗓 ممبر پرمیوم | ۳۰ روزه", "ممبر پرمیوم | 30 روزه", 888800, 100, 30000,
        _tg_prem_mem_desc("2 تا 24 ساعت", "عدم ریزش بین 20 تا 30 روز", "خارجی",
                          extra_notes=["📌 در زمان ثبت سفارش از جای دیگر سفارش ممبر ثبت نکنید در غیر این صورت مسئولیت هرگونه مورد با خودتان خواهد بود.",
                                      "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد."]), "tg_link"),
    _tg("tg_pm_mix_10_20d", "🌀 ممبر پرمیوم میکس | ۱۰-۲۰ روزه", "ممبر پرمیوم میکس ایرانی و خارجی | 10 تا 20 روزه", 1596200, 500, 10000,
        _tg_prem_mem_desc("12 تا 24 ساعت", "10-20 روز بدون ریزش", "میکس ایرانی و خارجی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_ir_7d", "🇮🇷 ممبر پرمیوم ایرانی | ۷ روزه", "ممبر پرمیوم ایرانی | 7 روزه", 1727800, 50, 10000,
        _tg_prem_mem_desc("3 تا 24 ساعت", "7 روز بدون ریزش", "ایرانی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_30d_fast", "⚡ ممبر پرمیوم | ۳۰ روز", "ممبر پرمیوم [ 30 روز ] ⚡️", 1739300, 100, 25000,
        _tg_prem_mem_desc("1 تا 24 ساعت", "نامشخص", "خارجی (ممبر ها به مدت 30 روز در کانال می مانند)"), "tg_link"),
    _tg("tg_pm_ir_10d", "🇮🇷 ممبر پرمیوم ایرانی | ۱۰ روزه", "ممبر پرمیوم ایرانی | 10 روزه", 2335700, 50, 10000,
        _tg_prem_mem_desc("3 تا 24 ساعت", "10 روز بدون ریزش", "ایرانی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_ir_14d", "🇮🇷 ممبر پرمیوم ایرانی | ۱۴ روزه", "ممبر پرمیوم ایرانی | 14 روزه", 2783600, 50, 10000,
        _tg_prem_mem_desc("3 تا 24 ساعت", "14 روز بدون ریزش", "ایرانی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_ir_30d", "🇮🇷 ممبر پرمیوم ایرانی | ۳۰ روزه", "ممبر پرمیوم ایرانی | 30 روزه", 4607300, 50, 10000,
        _tg_prem_mem_desc("3 تا 24 ساعت", "30 روز بدون ریزش", "ایرانی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_60d", "📅 ممبر پرمیوم | ۶۰ روزه", "ممبر پرمیوم | 60 روزه", 4888100, 10, 20000,
        _tg_prem_mem_desc("1 تا 24 ساعت", "نامشخص", "خارجی (ممبر ها به مدت 60 روز در کانال می مانند)",
                          extra_notes=["📌 هیچ تغییری در کانال بعد از سفارش انجام ندهید.",
                                      "📌 در زمان ثبت سفارش از جای دیگر سفارش ممبر ثبت نکنید در غیر این صورت مسئولیت هرگونه مورد با خودتان خواهد بود.",
                                      "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
    _tg("tg_pm_ir_90d", "🇮🇷 ممبر پرمیوم ایرانی | ۹۰ روزه", "ممبر پرمیوم ایرانی | 90 روزه", 9732800, 50, 10000,
        _tg_prem_mem_desc("3 تا 24 ساعت", "30 روز بدون ریزش", "ایرانی",
                          extra_notes=["📌 کانال عمومی باشد", "📌 کانال در حال ریزش نباشد. مسولیت ریزش ممبرهای قبلی کانال بر عهده مشتری می باشد.",
                                      _TG_NO_ORDER_TOGETHER]), "tg_link"),
]

# ─ گیفت تلگرام ─ (تعداد ثابت ۱)
def _tg_gift_desc():
    return _L("⏰ شروع: 2 تا 6 ساعت", "",
              "🔗 لینک: فقط ایدی حساب کاربری یا لینک کانال خود را وارد کنید.", "",
              "✅ در قسمت توضیحات لطفا اگر متنی مد نظرتون هست حتما بنویسید", "",
              "⚠️ در صورت ثبت ایدی اشتباه (مثلا ایدی گروه یا کانال) سفارش تکمیل شده و مبلغ به هیچ عنوان قابل برگشت نمیباشد.")

_TG_GIFT_SELF_NOTE = ("\n\n✅اگر قصد دارید از طرف خودتان این گیفت ارسال شود ، از سرویس استارز تلگرام استفاده کنید "
                      "و به اکانت خود در تلگرام ستاره اضافه کنید ، سپس وارد پی‌وی مورد نظر شوید و گزینه send gift را "
                      "انتخاب کنید و آیتم مورد نظر را برای ارسال به آن شخص انتخاب کنید.")

TG_GIFT = [
    _tg("tg_gift_bear", "🐻 گیفت عروسک خرس", "گیفت عروسک خرس 🐻", 81800, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_box", "🎁 گیفت کادو", "گیفت کادو 🎁", 135100, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_rose", "🌹 گیفت گل قرمز", "گیفت گل قرمز (Red Flower) 🌹", 135100, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_heart", "💖 گیفت قلب صورتی", "گیفت قلب صورتی 💖", 81800, 1, 1, _tg_gift_desc() + _TG_GIFT_SELF_NOTE, "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_bouquet", "💐 گیفت دسته گل", "گیفت دسته گل 💐", 266700, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_rocket", "🚀 گیفت موشک", "گیفت موشک 🚀", 266700, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_drink", "🍾 گیفت نوشیدنی", "گیفت نوشیدنی (Drink) 🍾", 266700, 1, 1, _tg_gift_desc() + _TG_GIFT_SELF_NOTE, "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_ring", "💍 گیفت انگشتر", "گیفت انگشتر 💍", 533300, 1, 1, _tg_gift_desc() + _TG_GIFT_SELF_NOTE, "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_diamond", "💎 گیفت الماس", "گیفت الماس 💎", 533300, 1, 1, _tg_gift_desc() + _TG_GIFT_SELF_NOTE, "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_cake", "🎂 گیفت کیک تولد", "گیفت کیک تولد 🎂", 266700, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_cup", "🏆 گیفت جام/کاپ", "گیفت جام/کاپ (Cup) 🏆", 533300, 1, 1, _tg_gift_desc() + _TG_GIFT_SELF_NOTE, "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_bday", "🎉 گیفت تولدت مبارک", "گیفت تولدت مبارک (Happy Birthday)", 1555300, 1, 1, _tg_gift_desc(), "tg_username", fixed=True, icon="🎁"),
    _tg("tg_gift_custom", "🌟 گیفت تلگرام دلخواه", "گیفت تلگرام دلخواه", 5332500, 50, 100000,
        _L("⏰ شروع: 2 تا 6 ساعت", "", "✅قیمت ذکر شده بابت هزینه ستاره های مورد نیاز گیفت است!", "",
           "✅در بخش لینک : آیدی شخص یا کانال مورد نظر", "", "✅در بخش تعداد ، تعداد ستاره مورد نیاز آن گیفت را وارد کنید", "",
           "⚠️ در صورت ثبت ایدی اشتباه (مثلا ایدی گروه یا کانال) سفارش تکمیل شده و مبلغ به هیچ عنوان قابل برگشت نمیباشد.", "",
           "⚠️اگر گیفت مورد نظر ستاره زیادتر از میزان ثبت شده داشته باشد ، سفارش لغو میشود. (اگر کمتر باشد ، هزینه ستاره اضافه به پنل بازگشت میخورد)"),
        "tg_username"),
]

# ─ ادز تلگرام ─
TG_ADS = [
    _tg("tg_ads_channel", "📺 حذف تبلیغ از کانال شما", "حذف ( تبلیغ ادز ) از کانال شما", 153600, 100, 1000000,
        _L("🕑 زمان شروع : 2 الی 12 ساعت", "👌 کیفیت سرویس : متوسط", "✅ زمان تکمیل : 24 تا 72 ساعت",
           "🔗 نمونه لینک : لینک کانال خودتون را ثبت کنید.", "🔗مثال: https://t.me/test", "",
           "نکات مهم :", "📌این سرویس روی تبلیغات تلگرامی که در کانال شما ظاهر می‌شوند، کلیک انجام می‌دهد."), "tg_link"),
    _tg("tg_ads_search", "🔍 حذف تبلیغ از سرچ تلگرام", "حذف ( تبلیغ ادز ) از سرچ تلگرام", 115200, 100, 1000000,
        _L("🕑 زمان شروع : 2 الی 12 ساعت", "👌 کیفیت سرویس : متوسط", "✅ زمان تکمیل : 24 تا 72 ساعت",
           "🔗 نمونه لینک : کلمه کلیدی را وارد کنید", "",
           "این سرویس برای کلمه کلیدی جستجوی مشخص شده، روی تبلیغات تلگرام کلیک می‌کند."), "tg_username"),
    _tg("tg_ads_bot", "🤖 حذف تبلیغ از ربات شما", "حذف ( تبلیغ ادز ) از ربات شما", 192000, 100, 200000,
        _L("🕑 زمان شروع : 2 الی 12 ساعت", "👌 کیفیت سرویس : متوسط", "✅ زمان تکمیل : 24 تا 72 ساعت",
           "🔗 نمونه لینک : لینک ربات را ثبت کنید.", "🔗مثال: https://t.me/yourbot", "",
           "نکات مهم :", "📌این سرویس روی تبلیغات تلگرامی که در ربات شما ظاهر می‌شوند، کلیک انجام می‌دهد."), "tg_link"),
    _tg("tg_ads_global", "🌍 ادز | گلوبال سرچ", "ادز تلگرام | گلوبال سرچ", 213300, 2000, 25000,
        _L("نمایش در رتبه‌ 1 سرچ تلگرام.", "", "تلگرام ادز چطور کار می‌کنه؟",
           "1. لینک کانال خودتون رو برای ما ارسال می‌کنید.", "2. اگر کانالتون تایید شد، لینک کانال‌های هدف رو برای ما می‌فرستید.",
           "3. تبلیغ شما ساخته می‌شه و بعد از تایید توسط تلگرام، در کانال‌های هدف نمایش داده می‌شه.", "👈 اجرا در ۷ تا ۱۰ روز"),
        "tg_link"),
    _tg("tg_ads_channels", "📢 ادز | کانال‌ها", "تلگرم ادز | کانال ها", 355500, 5000, 20000,
        _L("نمایش تبلیغ شما در کانال‌های مد نظرتان + نمایش در رتبه‌ 1 سرچ تلگرام.", "", "تلگرام ادز چطور کار می‌کنه؟",
           "1. لینک کانال خودتون رو برای ما ارسال می‌کنید.", "2. اگر کانالتون تایید شد، لینک کانال‌های هدف رو برای ما می‌فرستید.",
           "3. تبلیغ شما ساخته می‌شه و بعد از تایید توسط تلگرام، در کانال‌های هدف نمایش داده می‌شه.",
           "💰 2.5 میلیون تومان = 20,000 بازدید", "👈 اجرا در ۷ تا ۱۰ روز"), "tg_link"),
]

# ─ بازدید پست تلگرام ─
def _tg_view_desc(link_body):
    return _L("🕑 زمان شروع : 2 تا 5 ساعت", "🚀 سرعت ارسال : 1 الی 10کا در 24 ساعت", "🩸 میزان ریزش : ندارد",
              "👌 کیفیت سرویس : بالا", "✅ زمان تکمیل : 24 تا 48 ساعت", link_body)

TG_POST_VIEWS = [
    _tg("tg_view_1", "1️⃣ بازدید ۱ پست", "بازدید تلگرام 1 پست", 2700, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : https://t.me/abc/994"), "tg_post_link"),
    _tg("tg_view_5", "5️⃣ بازدید ۵ پست آخر", "بازدید تلگرام 5 پست آخر", 10300, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : لینک کانال را وارد کنید\n\nhttps://t.me/usename"), "tg_link"),
    _tg("tg_view_10", "🔟 بازدید ۱۰ پست آخر", "بازدید تلگرام 10 پست آخر", 21000, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : لینک کانال را وارد کنید\n\nhttps://t.me/usename"), "tg_link"),
    _tg("tg_view_20", "2️⃣0️⃣ بازدید ۲۰ پست آخر", "بازدید تلگرام 20 پست آخر", 41700, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : لینک کانال را وارد کنید\n\nhttps://t.me/usename"), "tg_link"),
    _tg("tg_view_50", "5️⃣0️⃣ بازدید ۵۰ پست آخر", "بازدید تلگرام 50 پست آخر", 104200, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : لینک کانال را وارد کنید\n\nhttps://t.me/usename"), "tg_link"),
    _tg("tg_view_100", "💯 بازدید ۱۰۰ پست آخر", "بازدید تلگرام 100 پست آخر", 178500, 10, 2147483647,
        _tg_view_desc("🔗 نمونه لینک : لینک کانال را وارد کنید\n\nhttps://t.me/usename"), "tg_link"),
    _tg("tg_view_200", "🥈 بازدید ۲۰۰ پست آخر", "بازدید تلگرام | 200 پست آخر", 312900, 10, 10000000,
        _L("🕑 زمان شروع : 0 تا 1 ساعت", "🚀 سرعت ارسال : 1 الی 10کا در 24 ساعت", "🩸 میزان ریزش : ندارد",
           "👌 کیفیت سرویس : بالا", "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : لینک کانال را وارد کنید",
           "https://t.me/abc"), "tg_link"),
    _tg("tg_view_500", "🥇 بازدید ۵۰۰ پست آخر", "بازدید تلگرام | 500 پست آخر", 586600, 10, 10000000,
        _L("🕑 زمان شروع : 0 تا 1 ساعت", "🚀 سرعت ارسال : 1 الی 10کا در 24 ساعت", "🩸 میزان ریزش : ندارد",
           "👌 کیفیت سرویس : بالا", "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : لینک کانال را وارد کنید",
           "https://t.me/abc"), "tg_link"),
]

# ─ بازدید خودکار تلگرام ─
TG_AUTO_VIEW_SETUP_GUIDE = _L(
    "1. وارد کردن نام کاربری", "ایدی کانال تلگرام خود را در بخش مربوطه وارد کنید.", "",
    "2. انتخاب پست‌ها", "تعداد پست‌های مورد نظر را به‌صورت زیر تعیین کنید:", "",
    "مثال 1: برای سفارش سین روی 10 پست اخیر:", 'مقدار "پست‌های جدید" را 0 و "پست‌های قدیمی" را 10 وارد کنید.', "",
    "مثال 2: برای سفارش سین روی 10 پست آینده:", 'مقدار "پست‌های جدید" را 10 و "پست‌های قدیمی" را 0 وارد کنید.', "",
    "⚠️ نکته:", "",
    'اگر مقدار "پست‌های جدید" را 0 وارد نکنید، سیستم آن را به‌صورت بی‌نهایت در نظر می‌گیرد. در این حالت، هر زمان که '
    "پست جدیدی منتشر کنید، به‌طور خودکار سفارش سین برای آن ثبت خواهد شد.", "",
    "3. ثبت خودکار سفارش", "سیستم به‌صورت خودکار پست‌های شما را بررسی کرده و سفارش سین را ثبت می‌کند.", "",
    "4. تعیین تعداد سین‌ها", "تعداد سین‌ها را در یک بازه مشخص کنید:", "",
    "به‌عنوان مثال، اگر حداقل را 1000 و حداکثر را 1500 وارد کنید، سیستم به‌صورت تصادفی عددی در این بازه برای هر پست "
    "انتخاب می‌کند. برای تعداد دقیق سین‌ها، حداقل و حداکثر را یکسان وارد کنید.", "",
    "5. هزینه و پیگیری سفارش",
    "هزینه سفارش بر اساس هر 1000 سین محاسبه شده و به‌صورت خودکار از حساب شما کسر می‌شود.")

TG_AUTO_VIEW = [
    _tg("tg_auto_view_1", "🤖 بازدید خودکار ۱ پست", "بازدید خودکار تلگرام 1 پست", 4000, 10, 2147483647,
        TG_AUTO_VIEW_SETUP_GUIDE, "tg_auto_view_setup", auto_view=True),
    _tg("tg_auto_view_5", "⚡ بازدید خودکار ۵ پست | سریع", "بازدید خودکار تلگرام 5 پست - سریع", 28300, 10, 1000000,
        _L("🕑 زمان شروع : 0 تا 1 ساعت", "🚀 سرعت ارسال : آنی ، گاهی با تأخیر", "🩸 میزان ریزش : ندارد",
           "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : لینک آخرین پست کانال", "", "https://t.me/abc/1234"),
        "tg_post_link"),
    _tg("tg_auto_view_10", "⚡ بازدید خودکار ۱۰ پست | سریع", "بازدید خودکار تلگرام 10 پست - سریع", 48400, 10, 1000000,
        _L("🕑 زمان شروع : 0 تا 1 ساعت", "🚀 سرعت ارسال : آنی ، گاهی با تأخیر", "🩸 میزان ریزش : ندارد",
           "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : لینک آخرین پست کانال", "", "https://t.me/abc/1234"),
        "tg_post_link"),
    _tg("tg_auto_view_20", "⚡ بازدید خودکار ۲۰ پست | سریع", "بازدید خودکار تلگرام 20 پست - سریع", 96700, 10, 1000000,
        _L("🕑 زمان شروع : 0 تا 1 ساعت", "🚀 سرعت ارسال : آنی ، گاهی با تأخیر", "🩸 میزان ریزش : ندارد",
           "✅ زمان تکمیل : 24 تا 48 ساعت", "🔗 نمونه لینک : لینک آخرین پست کانال", "", "https://t.me/abc/1234"),
        "tg_post_link"),
]
...
def add_telegram(data):
    """
    اپلیکیشن «🔵 تلگرام» را با همه‌ی بخش‌هایش اضافه می‌کند.
    چیزی را پاک یا جایگزین نمی‌کند؛ پوشه یا سرویسی که از قبل باشد دوباره ساخته نمی‌شود.
    """
    h = _cat_helpers(data)
    if h is None:
        return
    tg = find_app_folder(data, TELEGRAM_TITLE) or h.add_folder("root", TELEGRAM_TITLE, "بخش مورد نظر رو انتخاب کن 👇🔵")

    def add(parent, svc):
        h.add_service(parent, svc["key"], svc["title"], svc["label"], svc["price"], svc["mn"], svc["mx"],
                      svc["desc"], svc["platform"],
                      fixed_price=svc["price"] if svc["fixed"] else None, icon=svc["icon"])
        if svc["auto_view"] and svc["key"] in data["settings"]:
            data["settings"][svc["key"]].setdefault("need_auto_view", True)
    ...
    section(tg, "👥 ممبر ایرانی | گروه", "سرویس مورد نظر رو انتخاب کن 👇👥", TG_GROUP_IR)
    section(tg, "👤 ممبر کانال/گروه خارجی", "سرویس مورد نظر رو انتخاب کن 👇👤", TG_FOREIGN)
    section(tg, "🇮🇷 ممبر و بازدید ایرانی", "سرویس مورد نظر رو انتخاب کن 👇🇮🇷", TG_IR_MEMBER_VIEW)
    section(tg, "💎 اشتراک پرمیوم تلگرام", "مدت اشتراک رو انتخاب کن 👇💎", TG_PREMIUM_SUB)
    add(tg, TG_STARS)                                # فقط ۱ سرویس → مستقیم زیر تلگرام
    section(tg, "💎 ممبر پریمیوم [سرچ اول تلگرام باش]", "سرویس مورد نظر رو انتخاب کن 👇💎", TG_PREMIUM_MEMBER)
    section(tg, "🎁 گیفت تلگرام", "گیفت مورد نظر رو انتخاب کن 👇🎁", TG_GIFT)
    section(tg, "📢 ادز تلگرام", "سرویس مورد نظر رو انتخاب کن 👇📢", TG_ADS)
    section(tg, "👁 بازدید پست تلگرام", "سرویس مورد نظر رو انتخاب کن 👇👁", TG_POST_VIEWS)
    section(tg, "🤖 بازدید خودکار تلگرام", "سرویس مورد نظر رو انتخاب کن 👇🤖", TG_AUTO_VIEW)
...
def apply_one_time_fixes(data):
    ...
    if "telegram_v1" not in done:
        add_telegram(data)
        done.append("telegram_v1")
...
def handle_admin(chat_id, text, data, states):
    ...
    if state=="ap_wallet_uid":
        try:
            t=str(int(text.strip()))
            if t not in data["users"]:
                send(chat_id,"*⚠️ کاربر یافت نشد.*",BACK_BTN); states[uid]="admin"; return
            states[uid]=f"ap_wallet_amount|{t}"
            u=data["users"][t]
            send(chat_id,
                f"*👤 {u.get('first_name','?')} | 💼 موجودی فعلی: {u.get('wallet',0):,} تومان\n\n"
                f"مبلغ تغییر را به تومان بفرستید.\n"
                f"برای افزایش عدد مثبت (مثلاً 50000) و برای کاهش عدد منفی (مثلاً -50000-) بفرستید:*",BACK_BTN)
        except: send(chat_id,"*⚠️ آیدی اشتباه.*",BACK_BTN); states[uid]="admin"
        return

    if state.startswith("ap_wallet_amount|"):
        t = state.split("|",1)[1]
        states[uid]="admin"
        try:
            amount = to_int(text)
        except Exception:
            send(chat_id,"*⚠️ لطفاً فقط عدد بفرستید (برای کاهش با علامت منفی، مثلاً -50000). از منو دوباره شروع کنید.*",BACK_BTN); return
        u = data["users"].get(t)
        if not u:
            send(chat_id,"*⚠️ کاربر یافت نشد.*",BACK_BTN); return
        new_balance = u.get("wallet",0) + amount
        if new_balance < 0:
            send(chat_id,f"*⚠️ موجودی کافی نیست (موجودی فعلی: {u.get('wallet',0):,} تومان). از منو دوباره شروع کنید.*",BACK_BTN); return
        u["wallet"] = new_balance
        save_data(data)
        states[uid]="admin"
        verb = "افزایش" if amount>=0 else "کاهش"
        send(chat_id,f"*✅ کیف پول کاربر {t} {verb} یافت.\n💼 موجودی جدید: {new_balance:,} تومان*",BACK_BTN)
        try:
            send(int(t), f"*💼 موجودی کیف پول شما توسط پشتیبانی {verb} یافت.\n\n"
                          f"{'➕' if amount>=0 else '➖'} مبلغ: {abs(amount):,} تومان\n"
    ...
    if text=="👥 مدیریت کاربران":
        ...
        send(chat_id,f"*👥 کاربران: {len(data['users']):,} | مسدود: {bc}*",
             kb(["🚫 مسدود کردن کاربر","✅ رفع مسدودیت"],
                ...
                ["💰 افزایش/کاهش کیف پول"],["🔙 بازگشت"])); return
    ...
    if text=="💰 افزایش/کاهش کیف پول": states[uid]="ap_wallet_uid"; send(chat_id,"*آیدی عددی کاربر:*",BACK_BTN); return
...
LINK_VALIDATORS = {
    ...
    "tg_link":            validate_tg_link,
    "tg_post_link":       validate_tg_post_link,
    "tg_private_link":    validate_tg_private_link,
    "tg_link_or_private": validate_tg_link_or_private,
    "tg_username":        validate_tg_username,
    "tg_auto_view_setup": validate_tg_auto_view_setup,
}
LINK_HINTS = {
    ...
    "tg_post_link":       "https://t.me/username/1234",
    "tg_private_link":    "https://t.me/+xxxxxxxx",
    "tg_link_or_private": "https://t.me/username  یا  https://t.me/+xxxxxxxx",
    "tg_username":        "@username  یا  username  یا  https://t.me/username",
    "tg_auto_view_setup": "سه خط بفرستید:\nخط اول: یوزرنیم کانال (بدون @)\nخط دوم: تعداد پست‌های جدید (0 اگر نمی‌خواهید)\nخط سوم: تعداد پست‌های قدیمی",
}
        ...
        for upd in updates:
            try:
                ...
                if "successful_payment" in msg:
                    _payload = msg["successful_payment"].get("invoice_payload","")
                    if _payload.startswith("topup_"):
                        register_successful_topup(chat_id,_payload)
                    else:
                        register_successful_order(chat_id,_payload)
                ...
                if cur_state == "topup_fullname":
                    name = text.strip()
                    if len(name.split()) < 2 or len(name) < 5:
                        send(chat_id, "*⚠️ لطفاً نام و نام خانوادگی کامل را بنویسید (مثلاً: علی رضایی):*"); continue
                    data["users"].setdefault(uid, {}).setdefault("chat_id", chat_id)
                    data["users"][uid]["full_name"] = name
                    save_data(data)
                    states[uid] = "topup_phone"
                    send(chat_id, "*📞 حالا شماره تماس خودتون رو بنویسید (مثلاً: 09123456789):*", BACK_BTN_USER)
                    continue

                if cur_state == "topup_phone":
                    if not validate_phone_ir(text):
                        send(chat_id, "*⚠️ شماره تماس معتبر نیست. لطفاً یک شماره موبایل ایران بفرستید (مثلاً: 09123456789):*"); continue
                    data["users"].setdefault(uid, {})["phone"] = text.strip()
                    save_data(data)
                    states[uid] = "topup_amount"
                    send(chat_id, f"*✅ اطلاعات ثبت شد.\n\n💰 حالا مبلغ مورد نظر رو به تومان بنویسید (حداقل {TOPUP_MIN:,} تومان):*", BACK_BTN_USER)
                    continue

                if cur_state == "topup_amount":
                    try:
                        amount = to_int(text)
                        assert amount >= TOPUP_MIN
                    except Exception:
                        send(chat_id, f"*⚠️ لطفاً فقط عدد بفرستید (حداقل {TOPUP_MIN:,} تومان).*"); continue
                    states.pop(uid, None)
                    send_topup_invoice(chat_id, amount)
                    continue
