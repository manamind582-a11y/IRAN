...
CATALOG_VERSION      = 3      # با بالا بردن این عدد، بخش خدمات (کاتالوگ) هنگام استارت دوباره ساخته می‌شود
...
def seed_services(data):
    ...
    # پوشه آپارات
    counter += 1
    aparat_id = str(counter)
    nodes[aparat_id] = {
        "id": aparat_id, "parent": "root", "type": "folder",
        "title": "🎬 آپارات",
        "text": "خدمات آپارات رو انتخاب کن 👇🎬"
    }

    # واچ‌تایم‌های آپارات
    for emoji, hours, video, over, qty, price, finish in APARAT_WATCH_PACKAGES:
        counter += 1
        nid, key = str(counter), f"aparat_{hours}"
        label = f"{hours} ساعت واچ تایم آپارات"
        nodes[nid] = {
            "id": nid, "parent": aparat_id, "type": "service",
            "title": f"{emoji} {hours} ساعت واچ‌تایم", "key": key
        }
        settings[key] = {
            "enabled": True, "min": qty, "max": qty,
            "price_per_1000": max(1, round(price * 1000 / max(1, qty))), "fixed_price": price,
            "stock": 999999, "unit": "عدد", "platform": "aparat",
            "label": label, "head": f"{label} [ویدیو {video}]",
            "desc": APARAT_DESC.format(finish=finish, over=over, qty=qty, hours=hours),
        }

    # سرویس‌های اضافی آپارات (لایک / کامنت / بازنشر)
    for key, title, label, price1000, mn, mx, desc, platform in APARAT_EXTRA:
        ...
        settings[key] = {
            ...
            "price_per_1000": price1000,
            "stock": 999999, "unit": "عدد", "platform": platform,
            "label": label, "head": label,
            "desc": desc,
    ...
    # پوشه سروش
    counter += 1
    soroush_id = str(counter)
    nodes[soroush_id] = {
        "id": soroush_id, "parent": "root", "type": "folder",
        "title": "📱 سروش",
        "text": "خدمات سروش رو انتخاب کن 👇📱"
    }
    ...
    for key, title, label, price1000, mn, mx, desc in SOROUSH_SERVICES:
        ...
        nid = str(counter)
        nodes[nid] = {
            "id": nid, "parent": soroush_id, "type": "service",
            "title": title, "key": key
        }
        settings[key] = {
            "enabled": True, "min": mn, "max": mx,
            "price_per_1000": price1000,
            "stock": 999999, "unit": "عدد", "platform": "soroush",
            "label": label, "head": label,
            "desc": desc,
        }
